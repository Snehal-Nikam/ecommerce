    ...
    <div v-for="cartItem in cartItems" :key="cartItem.product.id" class="row mt-2 pt-3 justify-content-around">
      ...
      <div class="col-md-3 embed-responsive embed-responsive-16by9">
        <router-link :to="{ name: 'ShowDetails', params: { id: cartItem.product.id } }">
          <img v-bind:src="cartItem.product.imgURL" alt="" class="w-100 card-image-top embed-responsive-item" />
        </router-link>
      </div>
      ...
      <div class="col-md-5 px-3">
        <div class="card-block px-3">
          ...
          <p class="mb-0" id="item-quantity"> Quantity : <input size="1" class="p-0 h-25 border-bottom border-top-0 border-left-0 border-right-0" v-model="cartItem.quantity" />
          </p>
        </div>
        <p class="mb-0" id="item-total-price"> Total Price : <span class="font-weight-bold"> $ {{ cartItem.product.price * cartItem.quantity }}
          </span>
        </p>
        <br />
